...
int main(void)
{
	...
	unsigned char analog_data[5];							//アナログスティックの値を取得する配列
	...
	union wii_classic data;									//wiiコントローラーから受けとるデータ
	...
	MU2_SetChannel(Channel[SWITCH>>2][SWITCH<<2]);			//チャンネル設定

	while(1){
		set_controller();									//コントローラー初期化				
		get_value( data.buf );								//コントローラーから値取得
		get_analog( &data, analog_data );					//アナログスティックの値を取得
